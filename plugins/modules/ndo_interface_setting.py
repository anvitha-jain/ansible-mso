...
def main():
    ...
    if module.params.get("control") is not None:
        control = sorted(CONTROL_MAP.get(v) for v in module.params.get("control"))
    else:
        control = None
    ...
    if state == "present":
        ...
        if match:
            ...
            if access_macsec_policy:
                if mcp and mcp["admin_state"] and (mcp["admin_state"] == "disabled" or match.details.get("mcp", {}).get("adminState") == "disabled"):
                    mso.fail_json(msg="Access MACsec policy can only be set when MCP admin state is enabled.")
                else:
                    ...
                    if access_macsec_policy in existing_access_macsec_policy and (existing_access_macsec_policy[access_macsec_policy] != access_macsec_policy):
                        ops.append(
                            dict(
                                op="replace",
            ...
            if lldp:
                ...
                if lldp["status"] and lldp["status"] == "disabled":
                    if not (lldp["receive_state"] == "disabled" and lldp["transmit_state"] == "disabled"):
            ...
            if access_macsec_policy:
                if mcp and mcp["admin_state"] and mcp["admin_state"] == "disabled":
                    mso.fail_json(msg="Access MACsec policy can only be set when MCP admin state is enabled.")
                else:
